...
class pipeline:
    ...
    def load_imageclassification_model(self):
        model = tensorflow.lite.Interpreter(self.imgcls_path)
    ...
    def process_video(self,objectdetmodel,imgclsmodel,db):
        input_index = imgclsmodel.get_input_details()[0]["index"]
        output_index = imgclsmodel.get_output_details()[0]["index"]
        ...
        while True:
            ...
            if ret:
                ...
                img_rgb = cv2.cvtColor(resize_frame, cv2.COLOR_BGR2RGB)
                results = objectdetmodel.process(img_rgb)
                h, w, _ = resize_frame.shape
